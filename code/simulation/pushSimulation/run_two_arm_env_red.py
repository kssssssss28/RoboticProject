...
import time
from two_arms_env_red import GarbageSortingEnvOnlyRedV2
import time
import numpy as np
import random
...
with open('RL_model_trajectory.pkl', 'rb') as f:
    traj = pickle.load(f)
...
def main():
    ...
    random_numbers = [random.uniform(0.2, 0.55) for _ in range(number_of_garbage)]
    ...
    for i in range(number_of_garbage):
        ...
        move_list.append(predict_next_action(start_array1, pause_steps_num + process_num,stop = pause_steps_num))


    stay = predict_next_action(start_array1, 1000,stop = 1000)
    ...
    for episode in range(total_episodes):
        ...
        for step in range(max_steps_per_episode):
            ...
            if step % refresh == 0:
                print(f"Episode: {cnt + 1} is running")
                print("==="*10)
